...
def build_query(cell_bbs: Sequence[BoundingBox], cell_bbs_expanded: Sequence[BoundingBox], timeout: int, route_type: str) -> str:
    match route_type:
        case 'bus':
            return (
...
def is_way_for(tags: dict[str, str], route_type: str) -> bool:
    if route_type == 'bus':
...
def is_tram(tags: dict[str, str]) -> bool:
    return tags.get('tram') == 'yes'
...
class Overpass:
    ...
    async def query_relation(
        ...
        road_elements = tuple(e for e in maybe_road_elements if is_way_for(e['tags'], route_type))
        ...
        match route_type:
            case 'bus':
                elements_ex = (e for e in elements_ex if is_bus_related(e['tags']) or not is_rail_related(e['tags']))
            case 'tram':
                elements_ex = (e for e in elements_ex if is_tram(e['tags']) or is_rail_related(e['tags']))
