...
export const processRelationTags = (data) => {
	...
	if (data.nameOrRef)
		dummyDiv.appendChild(
			createElementFromHTML(`<tr><td colspan="2">${data.nameOrRef}</td></tr>`),
		);
	...
	for (const tag of interestingTags)
		if (data.tags[tag])
			dummyDiv.appendChild(
				createElementFromHTML(
					`<tr><td class="key">${tag}</td><td class="value">${data.tags[tag]}</td></tr>`,
				),
			);
...
export const processRouteStops = (data) => {
	...
	for (const collection of data.busStops) {
        ...
        <div class="route-summary-item">
            ...
            <div class="stop-name">${styleStopName(
							getBusCollectionName(collection),
						)}</div>
            <div class="stop-info">
                ${
									isPlatform
...
submitDownloadBtn.onclick = async () => {
	...
	fetch("/download_osm_change", {
		...
		.then((blob) => {
			...
			a.download = `relatify_${relationId}_${new Date()
				.toISOString()
				.replace(/:/g, "_")}.osc`;
			a.click();
		})
